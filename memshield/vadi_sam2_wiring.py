...
class VADIForwardFn:
    ...
    def forward_with_hiera(
        self,
        processed: Tensor,
        return_at: Iterable[int],
        hiera_at: Iterable[int],
        *,
        fpn_level: int = -1,
    ) -> Tuple[Dict[int, Tensor], Dict[int, Tensor]]:
        """Same as `__call__` but also returns per-frame Hiera tokens for
        ids in `hiera_at`. Used by Hiera feature-steering δ stage
        (codex Loop 3 R2 design #1, 2026-04-25).

        The Hiera tokens are extracted from `backbone_out["backbone_fpn"]
        [fpn_level]` (default -1 = deepest level, same as the
        vulnerability scorer's source). They carry gradient w.r.t.
        `processed[fid]` for any fid in `hiera_at`.

        Note: gradient checkpointing remains active. The returned Hiera
        tokens are a *forward output* of the checkpointed function and
        thus available for gradient-tracked downstream losses without
        breaking the checkpoint contract.

        Returns:
            (logits_by_t, hiera_by_t) — both dicts of `int → Tensor`
            with grad through `processed`.
        """
        if processed.dim() != 4 or processed.shape[-1] != 3:
            raise ValueError(
                f"processed must be [T, H, W, 3]; got "
        ...
        T_proc = int(processed.shape[0])
        return_set = {int(t) for t in return_at}
        hiera_set = {int(t) for t in hiera_at}
        bad = [t for t in (return_set | hiera_set) if not (0 <= t < T_proc)]
        if bad:
            raise ValueError(
                f"return_at|hiera_at ids out of [0, {T_proc}): {sorted(bad)}")
        ...
        with autocast_ctx:
            for fid in range(T_proc):
                ...
                if fid in hiera_set:
                    out_hiera[fid] = backbone_out["backbone_fpn"][
                        int(fpn_level)].float()

        missing_l = return_set - set(out_logits.keys())
        missing_h = hiera_set - set(out_hiera.keys())
        if missing_l or missing_h:
            raise RuntimeError(
                f"forward_with_hiera: missing logits {sorted(missing_l)} / "
                f"hiera {sorted(missing_h)}")
        return out_logits, out_hiera

    def forward_with_state(
        self,
        processed: Tensor,
        return_at: Iterable[int],
        state_at: Iterable[int],
    ) -> Tuple[Dict[int, Tensor], Dict[int, Tensor], Dict[int, Tensor]]:
        """Forward with logits AND SAM2 recurrent state at specified frames.

        Used by Decoy State Continuation polish (codex Loop 3 R3 design,
        2026-04-25). Returns three dicts:

          out_logits[t]  — pred mask logits at video resolution (existing)
          out_maskmem[t] — `track_step`'s `maskmem_features` for frame t.
                           Shape: SAM2 layout (typically [B, C, h_mem, w_mem]
                           with h_mem = w_mem = 64 for tiny). Carries
                           gradient w.r.t. `processed[t]`.
          out_obj_ptr[t] — `track_step`'s `obj_ptr` for frame t. Shape
                           [B, D] (typically [1, 256]).

        Both maskmem and obj_ptr come straight from the SAM2 dict that
        track_step already populates (see `_encode_new_memory` writes at
        sam2_base.py:808 and obj_ptr at :861). No internal SAM2 changes
        required — we just expose them.

        Note on autograd: `track_step` builds maskmem and obj_ptr inside
        the same forward graph that produces `pred_masks`. Gradient w.r.t.
        `processed[fid]` flows naturally; downstream losses (cosine
        distance to a teacher) drive δ updates through the SAM2 pipeline.
        """
        if processed.dim() != 4 or processed.shape[-1] != 3:
            raise ValueError(
                f"processed must be [T, H, W, 3]; got "
        ...
        T_proc = int(processed.shape[0])
        return_set = {int(t) for t in return_at}
        state_set = {int(t) for t in state_at}
        bad = [t for t in (return_set | state_set) if not (0 <= t < T_proc)]
        if bad:
            raise ValueError(
                f"return_at|state_at ids out of [0, {T_proc}): {sorted(bad)}")
        ...
        out_logits: Dict[int, Tensor] = {}
        out_maskmem: Dict[int, Tensor] = {}
        out_obj_ptr: Dict[int, Tensor] = {}
        ...
        with autocast_ctx:
            for fid in range(T_proc):
                ...
                if fid in return_set:
                    ...
                    out_logits[fid] = pred_vid.float()
                if fid in state_set:
                    mm = current_out.get("maskmem_features")
                    op = current_out.get("obj_ptr")
                    if mm is None or op is None:
                        raise RuntimeError(
                            f"forward_with_state: track_step at fid={fid} "
                            f"returned maskmem_features={mm is not None} "
                            f"obj_ptr={op is not None}; both required. "
                            f"Verify run_mem_encoder=True is being passed.")
                    out_maskmem[fid] = mm.float()
                    out_obj_ptr[fid] = op.float()

        missing_l = return_set - set(out_logits.keys())
        missing_s = state_set - set(out_maskmem.keys())
        if missing_l or missing_s:
            raise RuntimeError(
                f"forward_with_state: missing logits {sorted(missing_l)} / "
